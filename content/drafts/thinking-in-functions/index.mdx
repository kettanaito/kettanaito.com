...
Let's say we have an application that performs a lot of user operations based on their authentication status. What we may end up doing is checking the status in each of those functions:
...
You can spot how often we would need to repeat `user.isAuthenticated` each time we have to check the user authentication status.

```js
const latestPosts = withAuthentication(user => getLatestPosts(user))
```

Our `withAuthentication` function above is a high-order function because it accepts another function as an argument. Based on the name, this function ensures that the user is authenticated before fetching their latest posts. The brilliance of high-order functions is that the action supplied to them is dynamic.
...
Complexity is undesired, but unavoidable. Our task, as developers, is to prevent requirements' complexity becoming a programmatic complexity. For example, reusing things rather than introducing new things is one way to leverage this.
...
Unidirectional small functions are generally easier to test and swap with other functions. [...]
