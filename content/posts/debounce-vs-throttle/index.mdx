...
Hinges are tightened and the screws are steady—let's put our ingenious invention to a test!
...
Since `debounce` is called during the render, each Component's re-render will produce a new instance of a debounced `handleButtonClick` function, resulting into no effect being applied.
