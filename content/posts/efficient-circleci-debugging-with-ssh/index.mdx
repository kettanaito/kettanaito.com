...
I have been actively using [CircleCI](https://circleci.com/) in my projects for years now and I love it! However, I'm ashamed to admit that most of the failed jobs took me up to 1 hour to get right. By the time I saw CI pass, I would grew an unrivaled hatred towards the red color.
...
Thus, whenever a remote job fails, it's in our best interest to match the context in witch it failed as close as possible to provide a viable fix. Gladly, there is a way to connect to the very machine that run our job and explore its state. Enter SSH.
...
Originally designed by [Tatu Ylönen](#) in Helsinki University of Technology, SSH has got widely adopted and gradually became a standard in software engineering. Most of the websites you visit today use SSH for various purposes, like sendind and acceptind traffic, or transfering files.

In this tutorials we are primarily interested in these applications of SSH:
...
Those details are likely be different in your case, but your primarily interest should fall onto the machine's host (`100.26.97.124`) and port number (`64536`). Remember those. CircleCI would also print out a shorthand connection command, which we will skip for now.
...
When successfully executed, you will notice your terminal's working directory changing to the remote machine's name. Any commands issued in this process from now on are executed on the remote machine.
