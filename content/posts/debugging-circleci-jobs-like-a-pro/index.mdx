...
date: 2020-02-10
category: Education
title: 'Debugging CircleCI jobs like a pro using SSH'
description: Failed CircleCI job? Let's connect to the remote machine that ran it and do some next level debugging.
keywords:
  - circleci
  - ci
...
To minimize the amount of deviations between the remote computer that ran your CI job and your local machine, where you desperately try to achieve the same failing state, we are going to connect to the remote machine straight away using SSH connection.

## Running job with SSH

In order to debug a failed job using this technique the job needs to be run with SSH first. You can do that directly from the CircleCI UI:
...
Those details are likely be different in your case, but your primarily interest should fall onto the machine's host (`100.26.97.124`) and port number (`64536`). Remember those. CircleCI would also print out a shorthand connection command, which we will skip for now. First let's ensure that our local computer has an SSH key set up, otherwise any request to reach the remote machine will be denied.

## Into the remote!

As the article's title suggests, we are going to use an SSH connection to connect to the remote machine.
...
Connecting via SSH is like opening a door: you need a key. Such key bears an information about a connector, and helps the server to decide whether access should be granted. It's possible to have multiple keys stored in the SSH Agent, which is like a keychain for SSH.

Since this debugging technique involves SSH connection, you guessed it, you would also need a key. If you are already using SSH for your VCS (i.e. Git) you should use the same SSH public key attached to your account (the once that administrates CircleCI). If you don't have an SSH key, please follow the instructions below to create one.

#### Generate a new SSH key

There is a great tutorial called [Generating a new SSH key and adding it to the ssh-agent](https://help.github.com/en/github/authenticating-to-github/generating-a-new-ssh-key-and-adding-it-to-the-ssh-agent) from GitHub. Although it's primarily oriented on GitHub users, you can follow the same steps to generate a key, replacing GitHub-specific operations, like adding an SSH key in the user interface, with those relevant to your VCS.
...
#### Using SSH key when you're not the CircleCI admin?

- Might be worth mentioning and looking into.
...
When successfully executed, you will notice your terminal's working directory changed to the remote machine's name. Any commands issued in this process from now on are executed on the remote machine.
