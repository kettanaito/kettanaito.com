...
TypeScript is a powerful language superset. It helps me catch silly bugs, design and iterate on the software faster, and simply write better code.
...
In TypeScript, you can annotate about anything by assigning a type to it. For example, let's create a `print()` function that accepts a `message` and annotate that message as a string:
...
If we want for our `print()` function to also support printing numbers, we can turn the type of the `message` into a union, which is a combination of types.
...
The function can now indeed return either a string or a number but that return type _has no connection_ with the type of the `message` we pass in. We can pass a string and the return type of `print()` will still be `string | number` when it should really be just `string`.
...
Since we can store types using a generic, we can create different type behaviors based on that generic's value.
