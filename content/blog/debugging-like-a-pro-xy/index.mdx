...
The [Proxy](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Proxy) API is quite a powerfull piece of JavaScript. You can think of it as a spy on objects but that description doesn't do it justice. I've noticed I've been using it a lot lately, specifically when it comes to debugging rather tricky issues. Today, I will show you how.
...
This example wraps the `user` object in a proxy that has a `set` method defined. Whenever any property on the wrapped object is _set_ to the next value, that method is called. This allows you to A) spy on the setters; B) prevent a value change, if we want to.

> That `Reflect.set()` bit at the end of the `set` method translates to "perform the set operation as-is".

That's proxies in their essence—they allow you to listen to certain operations performed on the object. You can listen to many things too, like `set`, `get`, `apply`, `construct`, and many others. This becomes extremely powerful once you recall that almost everything in JavaScript is an object (arrays are objects, classes are objects, functions are objects, even literals like strings and numbers are, technically, objects).

## Who read the request body?

I had access to when a problematic request instance was created but I had to know what and why read its body. To be honest, the first thing I tried is simply stubbing the body reading methods of that request:
...
But I grew to dislike stubs over the years. Stubs _replace_ the functionality, disrupt the normal program flow, and always require you to go an extra step or two if you wish for the stub to perform things as-is (such as storing the reference to the original `request.text` to then calling it in the stub). They bring little value for a big price. I avoid stubs.

And—you guess it—that stub just refused to work. I wasn't quite in the mood to figure out why, I'd rather focus my mental capacity on the actual problem at hand.

Proxies are a much better solution to the problem. Naturally, here's what I did next:
...
> This snippet establishes a `Proxy` for each body reading method on the request and prints a stack trace to whatever agent that calls one of those methods.
