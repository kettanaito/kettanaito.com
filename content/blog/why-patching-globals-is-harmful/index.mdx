...
And I do get this appeal, I truly do. There is, however, a cost to every decision, and patching globals doesn't just have a cost that affects both users the maintainers, but also actively harms otherwise great APIs and the ecosystem around them. Now, to the concretics.
...
export async function UserProfile() {
  ...
  const profile = dedupe(await getUser())
...
The suggestions I'm proposing are also less invasive, as they operate on existing globals and behaviors as opposed to modifying them. Look, I've made a picture to illustrate how my mind perceives the explicit APIs vs those that patch globals:
