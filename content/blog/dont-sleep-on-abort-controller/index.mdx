...
- The `signal` property, which is an instance of [`AbortSignal`](https://developer.mozilla.org/en-US/docs/Web/API/AbortSignals). This is a pluggable part you can provide to any API to react to an abort even, and implement it accordingly. For example, providing it to a `fetch()` request will abort the request;
- The `.abort()` method that, when called, triggers the abort event.
...
You can provide the abort `signal` when adding event listeners to be automatically removed once the abort happens.
...
An `AbortController` instance is also much nicer to pass around, if a different part of your application is supposed to remove the listener.
...
In the example above, I'm adding a `useEffect` hook in React that introduces a bunch of event listeners with different purpose and implementation. Notice how in the clean up function I can remove _all of the added listeners_ by calling `controller.abort()` _once_. Neat!
...
// Create a readable stream that reads the given file.
const readStream = fs.createReadStream('large.json')

// Wrap the readable stream in `addAbortSignal`,
// which associates the `controller.signal` with it.
const cancelableReadStream = addAbortSignal(controller.signal, readStream)
```

> The `AbortSignal` you provide as the first argument to the `addAbortSignal` function can be _any_ abort signal instance, including those created via static methods like `AbortSignal.timeout()` or `AbortSignal.any()`.

Above, I am making a readable stream in Node.js cancelable by wrapping it in the `addAbortSignal` function and providing it the `controller.signal` reference. This means I can abort (i.e. destroy) that stream whenever I want by calling `controller.abort()`.
...
function makeCancelableTransaction(db) {
  return async (callback, { signal }: { signal?: AbortSignal }) => {
    await db.transaction((tx) => {
      // Rollback this transaction once the abort event is dispatched.
...
The `makeCancelableTransaction()` function accepts a database instance and returns an higher-order transaction function that now accepts a `signal` to be provided in the second argument.
...
controller.signal.addEventListener('abort', () => {
  console.log(signal.reason) // "user cancellation"
})
...
> `reason` can be any JavaScript value so you can pass strings, errors, or even objects.
